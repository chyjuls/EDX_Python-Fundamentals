...
# function that will search a key in BST
def binary_tree_search(node, key):
    # node in the initial call will be the root node of the tree
    # value of node will get change (as it is a recursive function)
    # by its left child (if key is smaller than node value) or
    # by its right child (if key is greater than node value)
    # key is value we needed to search

    # if key is equal to the node value
    if node.value == key:
        # return True
        return True

    # if key is lesser than current node value
    # we will now search the key in the left tree
    if key < node.value:
        # checking if left tree is not null
        if node.left:
            # call the function recursively
            # note, the node is node.left now, i.e. left child of node
            return binary_tree_search(node.left, key)
        # if no left child present return False
        else:
            return False

    # if key is greater than current node value
    # we will now search the key in the right tree
    if key > node.value:
        # checking if right tree is not null
        if node.right:
            # call the function recursively
            # note, the node is node.right now, i.e. right child of node
            return binary_tree_search(node.right, key)
        # if no right child is present return False
        else:
            return False
