...
def binary_search_year(list_dates, dates):
    list_dates.sort()
    if len(list_dates) == 0:
        return False
    mid = len(list_dates) // 2
    if list_dates[mid].year == dates:
        return True
    elif dates < list_dates[mid].year:
        return binary_search_year(list_dates[:mid], dates)

    else:
        return binary_search_year(list_dates[mid + 1:], dates)
